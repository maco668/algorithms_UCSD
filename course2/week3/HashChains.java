...
//                    elems.add(0, query.s);
				...
				if (!elems[hashValue].contains(query.s))
						elems[hashValue].add(0, query.s);					
                break;
            case "del":
				hashValue = hashFunc(query.s);	
                if (elems[hashValue].contains(query.s))
                    elems[hashValue].remove(query.s);
                break;
            case "find":
				hashValue = hashFunc(query.s);
                writeSearchResult(elems[hashValue].contains(query.s));
                break;
            case "check":
				hashValue = query.ind;
                if (elems[hashValue] != null){
					for (String cur : elems[hashValue])
                        out.print(cur + " ");
					out.println();
				} else
					out.println();
    ...
    public void processQueries() throws IOException {
		...
		//elems = (ArrayList<String>[]) new ArrayList<String>[bucketCount];
