...
struct TwoSatisfiability {
	...
	const vector<vector<int> > &graphR
		...
		DFS(graphR, visited, order, pre, post, clock);	
		...
		for(int i = n-1; i>=0; --i){
			...
			if(!visited[vertex]){
				CCnum.push_back({});
				Explore(implgraph, visited, CCnum, vertex);
			}
		}
		/* cout<<"SCC begin:"<<endl;
		for(auto ele: CCnum){
	...
	int &clock	
		...
		for(int i=0; i<graphR.size(); ++i){
			if(!visited[i]){
				//++cc;
				ExploreR(graphR,visited, order, pre, post, i, clock);
			}			
		}		
	}
	...
	int &clock	
		...
		for(int i=0; i<adj[vertex].size(); ++i){
			int w = adj[vertex][i];
			if(!visited[w])
				ExploreR(adj,visited, order, pre, post, w, clock);
		}
    ...
    bool isSatisfiable(vector<int>& result) {
		...
		vector<vector<int>> scc = SCC(implgraph, graphR);
